...
Python Discord bot for moderation, verification, modmail, private support tickets, staff applications, suggestion panels, NE Coins, shop rewards, anti-raid protection, server activity logs, auto-reactions, reaction roles, no-link channels, AFK statuses, configurable prefixes, and QOTD posting.

## Features

- Slash commands: `help`, `warn`, `mute`, `unmute`, `kick`, `ban`, `unban`, `addrole`, `removerole`, `role add`, `role remove`, `clear`, `modlogs`, `afk`, `prefix ...`, `verificationpanel`, `suggestionpanel`, `coins ...`, `rewards ...`, `shop ...`, `inventory`, `serverinfo post/edit`, `staffapplypanel` (`post` and `disable`), `ticket ...`, `qotd`, `embed`, `autoreact ...`, `reactionrole ...`, `nolink ...`, and `antiraid ...`
...
- PostgreSQL-only suggestion panel with modal submissions, discussion threads, persistent voting buttons, and staff moderation buttons
- PostgreSQL-only NE Coins economy with admin balance management, daily rewards, leaderboards, shop purchases, inventory, role rewards, manual fulfillment, and uploaded coin/item images
...
- PostgreSQL-backed persistence for modlogs, suggestions, votes, suggestion panel configuration, NE Coins, shop items, purchases, uploaded image URLs, auto-reactions, reaction roles, no-link channels, AFK statuses, command prefixes, and ticket settings when `DATABASE_URL` is configured
...
- Set `DATABASE_URL` for persistent PostgreSQL storage. The suggestion panel and NE Coins/shop system require PostgreSQL and do not use a local JSON fallback.
- Set `ASSET_CHANNEL_ID` to a private text channel if you want uploaded NE Coins and shop item images mirrored to a stable Discord CDN URL. Without it, the bot stores the original uploaded attachment URL.
- Administrators can run `/suggestionpanel suggestions_channel:#suggestions` to post the panel and save the target suggestions channel in PostgreSQL.
- Administrators can run `/coins image`, `/coins give`, `/rewards config`, and `/shop create` to manage NE Coins and rewards fully inside Discord.
- Without `DATABASE_URL`, the suggestion panel and NE Coins/shop system are unavailable. Auto-reaction rules are stored in `autoreact_data.json`, reaction roles are stored in `reaction_roles.json`, no-link channel rules are stored in `no_link_channels.json`, AFK statuses are stored in `afk_data.json`, command prefixes are stored in `prefix_data.json`, ticket settings are stored in `ticket_config.json`, and moderation logs stay in memory until restart.
